...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        p = self.encoder(x)
        mu = self.mu_layer(p)
        logvar = self.logvar_layer(p)
        z = reparametrize(mu, logvar) # sample latent
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        p = self.encoder(torch.cat((self.flatten(x), c), dim=1))
        mu = self.mu_layer(p)
        logvar = self.logvar_layer(p)
        z = reparametrize(mu, logvar) # N,Z
        x_hat = self.decoder(torch.cat((z, c), dim=1)) # N,Z
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
        return x_hat, mu, logvar
...
def loss_function(x_hat, x, mu, logvar):
    ...
    latent_loss = -0.5 * torch.sum(1 + logvar - mu ** 2 - torch.exp(logvar))
