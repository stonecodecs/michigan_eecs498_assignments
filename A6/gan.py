...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cpu"):
    ...
    # expand to [0,2], then zero-center to [-1,1]
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.sum(0.5 * (scores_real - 1) ** 2) / N
    loss += torch.sum(0.5 * (scores_fake) ** 2) / N
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def ls_generator_loss(scores_fake):
    ...
    # Replace "pass" statement with your code
    N = scores_fake.shape[0]
    loss = torch.sum(0.5 * (scores_fake - 1) ** 2)
    loss /= N
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
...
def build_dc_classifier():
    ...
    model = nn.Sequential(
        ...
        nn.LeakyReLU(negative_slope=0.01),
        nn.MaxPool2d(2, stride=2),              # HW / 4 = 12x12
        nn.Conv2d(32, 64, 5, stride=1),
        nn.LeakyReLU(negative_slope=0.01),
        nn.MaxPool2d(2, stride=2),              # HW / 16 = 4x4
        nn.Flatten(),
