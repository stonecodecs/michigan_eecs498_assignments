...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    H, C = W2.shape
    hidden = torch.maximum(                  # W1(D,H) X(N,D) => (N,H)
        torch.zeros(N,H, device="cuda"),
        (X @ W1) + b1)                     
    scores = hidden @ W2 + b2          
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    stabilizer = torch.max(scores, keepdim=True, dim=1)[0]
    logits = torch.exp(scores - stabilizer)
    probas = logits / torch.sum(logits, keepdim=True, dim=1)
    loss = torch.sum(-torch.log(probas[torch.arange(0, N), y]))
    loss /= N
    ...
    d_L = 1.
    d_Z2 = probas * d_L
    d_Z2[torch.arange(0, N), y] -= 1 # for matching i=y
    Z1 = X @ W1 + b1
    A1 = (Z1 > 0).float() * Z1 # (N,H)
    d_A1 = d_Z2 @ W2.t() # (N,C) @ (C,H) => (N,H)
    d_b2 = torch.sum(d_Z2, dim=0) / N
    d_W2 = A1.t() @ d_Z2 / N # (H,N) @ (N,C) => (H,C) 
    d_Z1 = (Z1 > 0).float() * d_A1 # (N,H) @ (N,H)
    d_b1 = torch.sum(d_Z1, dim=0) / N # (H,) sum over dim=0
    d_W1 = X.t() @ d_Z1 / N # (D,N) @ (N,H) => (D,H)

    # probas is basically A2
    ...
    # grads['W2'] = (A1.t() @ probas) / N # => (H,C)
    # grads['b2'] = torch.sum(probas, dim=0) / N # => (C,)

    # grads['W1'] = X.t() @ (mask * (W2 @ probas.t()).t()) / N # => (D,H)
    # grads['b1'] = torch.sum(mask * (W2 @ probas.t()).t(), dim=0) / N # => (H,)
...
def nn_predict(
    ...
    stabilizer = torch.max(Z2, keepdim=True, dim=1)[0]
    A2 = torch.exp(Z2 - stabilizer)
    y_scores = A2 / torch.sum(A2, keepdim=True, dim=1)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e0, 1e-1, 1.1, 8e-1]
    hidden_sizes = [128,192,256,512]
    regularization_strengths = [1e-6, 1e-3]
    learning_rate_decays = [1.0, 0.99]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hs in hidden_sizes:
            for rs in regularization_strengths:
                for decay in learning_rate_decays:
                    print(lr, hs, rs, decay, '\n')
                    nn = TwoLayerNet(3 * 32 * 32, hs, 10, 
                                      device=data_dict['X_train'].device,
                                      dtype=data_dict['X_train'].dtype)
                    stats = nn.train(data_dict['X_train'], data_dict['y_train'],
