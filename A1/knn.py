...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        for j in range(num_test):
            ...
            dist = torch.sum(((te_flat - tr_flat) ** 2))
            dists[i,j] = dist 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i, xtr in enumerate(x_train):
        ...
        x_train_flat = xtr.view(-1)
        dist = torch.sum((x_train_flat - x_test_flat) ** 2, dim=1)
        dists[i, :] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xtr = x_train.view(num_train, -1)
    xte = x_test.view(num_test, -1)

    # notice that sum(x0-x1)^2 = (x0)^2 - 2(x0x1) + (x1)^2
    xtr_2 = torch.sum(xtr ** 2, dim=1, keepdim=True)
    xte_2 = torch.sum(xte ** 2, dim=1, keepdim=True).t()
    correction = 2 * torch.matmul(xtr, xte.t())
    dists = xtr_2 + xte_2 - correction # double broadcasting onto "correction"
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    top_k_val, top_k_idx = torch.topk(dists, k, dim=0, largest=False)
    top_k_y = y_train[top_k_idx]
    y_pred, _ = torch.mode(top_k_y, dim=0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
        for k in k_choices:
            ...
            clf = KnnClassifier(train_x, train_y)
            if k not in k_to_accuracies:
                k_to_accuracies[k] = []
            k_to_accuracies[k].append(clf.check_accuracy(
                x_train_folds[i], y_train_folds[i], k=k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    acc_list = [k_to_accuracies[key] for key in k_to_accuracies.keys()]
    ktensor = torch.tensor(acc_list)
    best_idx = torch.argmax(torch.mean(ktensor, dim=1))
    k_list = [key for key in k_to_accuracies.keys()]
