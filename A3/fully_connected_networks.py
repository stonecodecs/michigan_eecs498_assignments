...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # Replace "pass" statement with your code
        self.params = {
            "W1": torch.normal(0, weight_scale, 
              size=(input_dim, hidden_dim),
              dtype=dtype,
              device=device),
            "b1": torch.zeros((hidden_dim), device=device),
            "W2": torch.normal(0, weight_scale, 
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        h1, cache_h1 = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        scores, cache_scores = Linear.forward(h1 ,self.params["W2"], self.params["b2"])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        # gradients [d_scores is shape (N,C)]
        d_x2, d_W2, d_b2 = Linear.backward(d_scores, cache_scores)
        grads["W2"] = d_W2 + (self.reg * 2 * self.params["W2"])
        grads["b2"] = d_b2
        d_x1, d_W1, d_b1 = Linear_ReLU.backward(d_x2, cache_h1)
        grads["W1"] = d_W1 + (self.reg * 2 * self.params["W1"])
        grads["b1"] = d_b1
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            hidden_dim = hidden_dims[i] if i != len(hidden_dims) else num_classes
                
            self.params["W" + str(i + 1)] = torch.normal(
                0, weight_scale, size=(prev_dim, hidden_dim),
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            Z, cache = Layer.forward(Z, W, b)
            cache_stack.append(cache)
            # better performance if Dropout not applied at last layer
            if self.use_dropout and i < self.num_layers - 1:
              Z, cache_drop = Dropout.forward(Z, self.dropout_param)
              cache_stack.append(cache_drop)
        ...
        for i in range(self.num_layers - 1, -1, -1):
            ...
            Layer = Linear if i == self.num_layers - 1 else Linear_ReLU
            # better performance when dropout isn't applied at the last layer
            if self.use_dropout and i != self.num_layers - 1:
              dZ = Dropout.backward(dZ, cache_stack.pop())
            dX, dW, db = Layer.backward(dZ, cache_stack.pop())
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    # using momentum in HOML book
    # EECS 458 note's implementation doesn't work??
...
def rmsprop(w, dw, config=None):
    ...
    config["cache"] = s
    next_w = w - config["learning_rate"] * dw / torch.sqrt(s + config["epsilon"])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    config["m"] = m
    config["v"] = v
    config['t'] += 1 # t must first be 1
    # bias correctored
    m_hat = m / (1 - b1 ** config['t'])
    v_hat = v / (1 - b2 ** config['t'])
