...
# my own im2col implementation for Conv.forward
# def im2col(x, w, b, stride):
    """
        A naive implementation of the forward pass for a convolutional layer.
        The input consists of N data points, each with C channels, height H and
        width W. We convolve each input with F different filters, where each
        filter spans all C channels and has height HH and width WW. Padding 
        is expected to already have been performed onto x.
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for num_k, kernel in enumerate(w): # concatenate filter convs  for final 'out'
            fmap = torch.zeros(N, 1, _H, _W, dtype=x.dtype, device=x.device)
            for i in range(0, H, stride):
                for j in range(0, W, stride):
                  # N,C,HH,WW (across all batches)
                  op_area = x_padded[:,:,i:i+HH,j:j+WW]
                  # kernel is C,HH,WW (broadcasts through all channels)
                  kernel_area = op_area * kernel
                  fmap[:, 0, (i//stride), (j//stride)] = torch.sum(
                      kernel_area, dim=(1,2,3))
            if out is None:
              out = fmap + b[num_k]
            else:
              out = torch.cat((out, fmap + b[num_k]), dim=1)
    ...
    def backward(dout, cache):
        ...
        x_padded = torch.nn.functional.pad(x, (pad,) * 4,
                                           mode='constant')
        dx_padded = torch.zeros_like(x_padded) # shrink to dx.shape later

        for img in range(N):
            for f in range(F):
                for i in range(0, H, stride):
                    for j in range(0, W, stride):
                        dx_padded[img, :, i:i+HH, j:j+WW] += w[f] \
                        * dout[img, f, i // stride, j // stride]
                        dw[f] += x_padded[img, :, i:i+HH, j:j+WW] \
                        * dout[img, f, i // stride, j // stride]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros((N,C, _H,_W), dtype=x.dtype, device=x.device)

        for i in range(_H):
            for j in range(_W):
                ii = i * stride
                jj = j * stride
                flat = x[:,:,ii:ii+ph,jj:jj+pw].reshape(N,C,-1)
                out[:,:,i,j] = torch.max(flat, dim=2)[0]
          ...
          for c in range(C):
            for i in range(0, H, stride):
                for j in range(0, W, stride):
                    maxval = float('-inf')
                    maxidx = 0
                    for row in range(i, i+ph):
                        for col in range(j, j+pw):
                            if x[img,c,row,col] > maxval:
                                maxval = x[img,c,row,col]
                                maxidx = (row, col)

                    dx[img,c,maxidx[0],maxidx[1]] += dout[img,c,maxidx[0]//stride,maxidx[1]//stride]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params["b1"] = torch.zeros(
            num_filters, dtype=self.dtype, device=device,
        )
        ...
        self.params["b2"] = torch.zeros(
            hidden_dim, dtype=self.dtype, device=device,
        )
        ...
        self.params["b3"] = torch.zeros(
            num_classes, dtype=self.dtype, device=device,
        )
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        z1, cache_conv = Conv_ReLU_Pool.forward(
            X, W1, b1, conv_param=conv_param, pool_param=pool_param)
        z2, cache_h1 = Linear_ReLU.forward(z1, W2, b2)
        scores, cache_h2 = Linear.forward(z2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        W_tensors = [
            self.params["W" + str(n)] ** 2 
            for n in range(1, 4)
        ]
        W_sums = [torch.sum(W) for W in W_tensors]
        cumsum_l2 = torch.sum(torch.tensor(W_sums))
        loss += self.reg * cumsum_l2

        d_z2, d_W3, d_b3 = Linear.backward(d_scores, cache_h2)
        d_z1, d_W2, d_b2 = Linear_ReLU.backward(d_z2, cache_h1)
        d_X, d_W1, d_b1 = Conv_ReLU_Pool.backward(d_z1, cache_conv)

        grads["W3"] = d_W3 + (self.reg * torch.sum(W3) * 2)
        grads["W2"] = d_W2 + (self.reg * torch.sum(W2) * 2)
        grads["W1"] = d_W1 + (self.reg * torch.sum(W1) * 2)
        grads["b3"] = d_b3
        grads["b2"] = d_b2
        grads["b1"] = d_b1
        ###################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        # 3x3 kernels, stride=1, same size as nput, max pooling 2x2 stride=2
        C, H, W = input_dims

        # all convolutional layers
        for i in range(self.num_layers - 1):
            prev_filters = num_filters[i - 1] if i > 0 else C

            if weight_scale == "kaiming":
                self.params["W" + str(i + 1)] = kaiming_initializer(
                    prev_filters, num_filters[i], K=3, relu=True,
                    dtype=self.dtype, device=device)
            else:
                self.params["W" + str(i + 1)] = torch.normal(
                    0, weight_scale, size=(num_filters[i], prev_filters, 3, 3),
                    dtype=self.dtype, device=device)
                
            self.params["b" + str(i + 1)] = torch.zeros(
                num_filters[i], dtype=self.dtype, device=device)
        ...
        if weight_scale == "kaiming":
            self.params["W" + str(self.num_layers)] = kaiming_initializer(
                num_filters[self.num_layers - 2] * H * W, num_classes,
                relu=True, dtype=self.dtype, device=device)
        else:
            self.params["W" + str(self.num_layers)] = torch.normal(
                0, weight_scale, 
                size=(num_filters[self.num_layers - 2] * H * W, num_classes),
                dtype=dtype,
                device=device) 

        self.params["b" + str(self.num_layers)] = torch.zeros(
    ...
    def loss(self, X, y=None):
        ...
        layer_stack = []
        for i in range(self.num_layers - 1):
            Layer = Conv_BatchNorm_ReLU if self.batchnorm else Conv_ReLU
            ...
            if i in self.max_pools:
               Z, cache = FastMaxPool.forward(Z, pool_param)
               cache_stack.append(cache)
               layer_stack.append(FastMaxPool)

        scores, cache_final = Linear.forward(
            Z,
        ...
        # Replace "pass" statement with your code
        loss, d_scores = softmax_loss(scores, y)
        reg_loss = sum(torch.sum(self.params["W" + str(i + 1)] ** 2)
                        for i in range(self.num_layers))
        loss += self.reg * reg_loss

        dx, dw, db = Linear.backward(d_scores, cache_final)
        grads["W" + str(self.num_layers)] = dw + \
            (self.reg * 2 * self.params["W" + str(self.num_layers)])
        grads["b" + str(self.num_layers)] = db
        dZ = dx
        for i in range(self.num_layers - 2, -1, -1):
            curr_Wb = self.params["W" + str(i + 1)]
            if i in self.max_pools:
                Layer = layer_stack.pop()
                dZ = Layer.backward(dZ, cache_stack.pop())
            Layer = layer_stack.pop()

            if self.batchnorm: # Layer is Conv_BatchNorm_ReLU
                dZ, dW, db, dgamma, dbeta = Layer.backward(dZ, cache_stack.pop())
                grads["gamma" + str(i + 1)] = dgamma
                grads["beta" + str(i + 1)] = dbeta
            else: # layer is Conv_ReLU
                dZ, dW, db = Layer.backward(dZ, cache_stack.pop())

            grads["W" + str(i + 1)] = dW + (self.reg * 2 * curr_Wb)
            grads["b" + str(i + 1)] = db
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    # best one so far: 71.15% valid, 70.53% test
    model = DeepConvNet(
        input_dims=(3,32,32),
        num_classes=10,
    ...
    solver = Solver(model, data_dict,
           ...
           update_rule=adam, lr_decay=.95, device=device)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            ##################################################################
            # Replace "pass" statement with your code
            mean   = (1./ N) * torch.sum(x, dim=0) 
            x_bar  = x - mean
            x_bar2 = x_bar ** 2
            var    = (1./ N) * torch.sum(x_bar2, dim=0)
            ...
            cache = (x, x_bar, x_hat, istd, var, std, eps, gamma, bn_param)
            return out, cache
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        # mean, var are running averages when testing
        x, x_bar, x_hat, istd, var, std, eps, gamma, bn_param = cache
        mode = bn_param["mode"]
        ...
        if mode == "train":
            ...
            # children of dx_hat
            dx_bar = istd  * dx_hat
            distd  = torch.sum(x_bar * dx_hat, dim=0)
            
            # children of dx_bar
            #  + lineage of istd
            dstd    = (-1. / (std ** 2)) * distd
            dvar    = (1. / (2 * torch.sqrt(var + eps))) * dstd
            dx_bar2 = 1./N * dvar
            dx_bar  += (2 * x_bar * dx_bar2)
            dmean   = -1./ N * torch.sum(dx_bar, dim=0)
            dx      = dx_bar + dmean
    ...
    def backward_alt(dout, cache):
        ...
        if mode == "train":
            ...
            dx = (N * dx_hat - torch.sum(dx_hat, dim=0) - x_hat * torch.sum(dx_hat * x_hat, dim=0)) / N / std
            
            # dvar = torch.sum(
            #     dx_hat * (x - mean) * (-1/2) * (var + eps) ** (-3/2), dim=0)
            # dmean = torch.sum(dx_hat * -1 / torch.sqrt(var + eps), dim=0) + dvar * torch.mean((-2.*(x-mean)), dim=0)
            # dx = dx_hat * (1. / torch.sqrt(var + eps)) + dvar * 2 * (x-mean) / N + dmean / N
        else: 
            raise ValueError('Invalid alt_backward batchnorm mode "%s"' % mode)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ################################################################
        # Replace "pass" statement with your code
        # description wrong, bn_param only providse MODE
        N, C, H, W = x.shape
        # C first to make compatible with gamma and beta (C,) => (C,D);
        # transpose to account for BatchNorm taking mean/sums with dim=0
        #   => mean((D,C), dim=0) => (C,) average by channel
        x_reduced = x.permute(1,0,2,3).reshape(C, -1).t()
        x_normal, cache = BatchNorm.forward(x_reduced, gamma, beta, bn_param)
        out = x_normal.t().reshape(C, N, H, W).permute(1,0,2,3)
        # BatchNorm output is (D,C)
        # transpose to get in "channel-first" form
        # then reshape to make sure data is properly shaped
        # permute to get in original order
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dout_reduced = dout.permute(1,0,2,3).reshape(C, -1).t()
        dx, dgamma, dbeta = BatchNorm.backward(dout_reduced, cache)
